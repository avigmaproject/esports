import { Country, IndexedCountries, ProjectTheme } from "./models/settings";
...
export const indexedCountries: IndexedCountries = countries.reduce(
  (acc: IndexedCountries, curr: Country) => {
    if (!acc[curr.countryCode]) {
      acc[curr.countryCode] = curr.countryNameEn;
    }
    return acc;
  },
  {},
...
export const mapCountries = countries.map((item: Country) => ({
  label: item.countryNameEn,
  value: item.countryCode,
}));

export const mapTimezones = timezones.map(item => ({
  label: item,
  value: item,
}));
